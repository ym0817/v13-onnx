...
void utils::visualizeDetection(cv::Mat& im, std::vector<YoloResult>& results,
    const std::vector<std::string>& classNames)
{
    std::vector<cv::Scalar> hscolors;
    hscolors.push_back(cv::Scalar(0, 0, 255));     // person
    hscolors.push_back(cv::Scalar(255, 0, 0));     // bicycle
    hscolors.push_back(cv::Scalar(0, 255, 0));     // car
    hscolors.push_back(cv::Scalar(128, 128, 0));     // motorcycle
    hscolors.push_back(cv::Scalar(0, 128, 128));     // bus
    hscolors.push_back(cv::Scalar(128, 0, 128));     // track
    for (const YoloResult& result : results)
    {
        ...
        cv::rectangle(im, result.box, hscolors[classId], 2);
        //        cv::rectangle(image,
